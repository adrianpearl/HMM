...
class HMM:
	...
	def tocsvs(self, logposteriors):
		print("saving model")
		np.savetxt("model_%s/pi.csv" % (self.model_name), self.pi, delimiter=",", fmt="%1.5g")
		np.savetxt("model_%s/A.csv" % (self.model_name), self.A, delimiter=",", fmt="%1.5g")
		np.savetxt("model_%s/logps.csv" % (self.model_name), logposteriors, delimiter="\n", fmt="%1.5g")
		self.states_tocsvs()
...
class GaussianMixtureHMM(HMM):
	...
	def states_tocsvs(self):
		for state in range(self.states):
			...
			np.savetxt("model_%s/state%d_means.csv" % (self.model_name, state), means, delimiter=",", fmt="%1.5g")
			np.savetxt("model_%s/state%d_covs.csv" % (self.model_name, state), covs, delimiter=",", fmt="%1.5g")
			np.savetxt("model_%s/state%d_cops.csv" % (self.model_name, state), cops, delimiter=",", fmt="%1.5g")

	def loadModel(name):
		loadA = np.loadtxt("model_" + name + "/A.csv", delimiter=",")
		loadpi = np.loadtxt("model_" + name + "/pi.csv", delimiter=",")
		loadB = []
		numstates = loadA.shape[0]
		for state in range(numstates):
			means = np.loadtxt("model_%s/state%d_means.csv" % (name, state), delimiter=",")
			covs = np.loadtxt("model_%s/state%d_covs.csv" % (name, state), delimiter=",")
			cops = np.loadtxt("model_%s/state%d_cops.csv" % (name, state), delimiter=",")
...
class GaussianHMM(HMM):
	...
	def logp_obs_states(self, obs):
		...
		for state in range(self.states):
			...
			prob = mvGaussian(obs, mean, np.diag(cov))
	...
	def estimate_state_distributions(self, X_train, sequence_lengths):
		...
		for i in range(0, self.states):
			...
			"""component means - initialize to random observations"""
			randomobs = np.random.randint(X_train.shape[0], size=1)
			mean = X_train[randomobs, :]
			self.B.append((mean, cov))
		...
		for state in range(self.states):
			if self.feedback:
				print("state: ", state)
			state_obs = X_train[state_sequence == state]
			mean = np.mean(state_obs, axis=0)
			cov = np.diag(state_obs.transpose().dot(state_obs)/state_obs.shape[0])
			self.B[state] = (mean, cov)

	"""
	Input:	observation sequences and sequence lengths
	Output:	numpy arrays gamma(observations x number of states)
			and xi(number of states x number of states)
	gamma[t][i] = p(O(t) generated from state i | O sequence, model)
	xi[i][j] = total expected transitions from state i to state j in all sequences
	"""
	def gamma_xi(self, X_train, sequence_lengths):
	...
	def reestimate_lambda(self, X_train, sequence_lengths):
		...
		for state in range(self.states):
			...
			mean = np.sum(np.multiply(X_train, gammastate[:, np.newaxis]), axis=0) / np.sum(gammastate)
			diff_sq = np.square(X_train - mean)
			cov = np.sum(np.multiply(diff_sq, gammastate[:, np.newaxis]), axis=0) / np.sum(gammastate)
	...
	def states_tocsvs(self):
		for state in range(self.states):
			means = self.B[state][0]
			covs = self.B[state][1]

			np.savetxt("model_%s/state%d_mean.csv" % (self.model_name, state), means, delimiter=",", fmt="%1.5g")
			np.savetxt("model_%s/state%d_cov.csv" % (self.model_name, state), covs, delimiter=",", fmt="%1.5g")
	...
	def loadModel(name):
		...
		numstates = loadA.shape[0]
		for state in range(numstates):
			means = np.loadtxt("model_%s/state%d_mean.csv" % (name, state), delimiter=",")
			covs = np.loadtxt("model_%s/state%d_cov.csv" % (name, state), delimiter=",")
			loadB.append((means, covs))
		model = GaussianHMM(name, numstates)
		model.A = loadA
		model.B = loadB
		model.pi = loadpi
		return model
